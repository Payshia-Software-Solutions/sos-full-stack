      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="w-full justify-between bg-slate-950 border-slate-800 text-slate-100 hover:bg-slate-950 hover:text-slate-100 focus:ring-0 focus:ring-offset-0 focus:border-primary h-10 text-sm font-normal"
        ...
        <PopoverContent className="w-[--radix-popover-trigger-width] p-0 bg-slate-900 border-slate-800 text-slate-100" side="bottom" align="start">
          <Command className="bg-slate-900 text-slate-100">
            <CommandInput placeholder="Search bank..." className="text-slate-100 placeholder:text-slate-500 border-b border-slate-800" />
            <CommandEmpty className="p-2 text-sm text-slate-400 text-center">No bank found.</CommandEmpty>
              ...
              <CommandGroup>
                {banks.map((bank) => (
                  <CommandItem
                    ...
                    className="hover:bg-slate-850 focus:bg-slate-800 text-slate-100 cursor-pointer flex items-center justify-between"
                  >
    ...
    <div className="p-4 md:p-8 space-y-8 pb-20">
      ...
      {balance && (
        <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6">
          <div className="p-3 sm:p-4 md:p-6 rounded-xl border border-slate-800 bg-slate-950/40 text-center shadow-md flex flex-col justify-center">
            <p className="text-[10px] sm:text-xs md:text-sm text-muted-foreground font-medium truncate">Total Fee</p>
            <p className="text-sm sm:text-lg md:text-xl lg:text-2xl font-bold text-slate-100 mt-1 truncate">
          ...
          <div className="p-3 sm:p-4 md:p-6 rounded-xl border border-slate-800 bg-slate-950/40 text-center shadow-md flex flex-col justify-center">
            ...
            <p className="text-sm sm:text-lg md:text-xl lg:text-2xl font-bold text-green-500 mt-1 truncate">
      ...
      {enrollments.length > 0 && (
        <Card className="shadow-lg border-slate-850 bg-slate-900/60 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-primary/5 to-transparent border-b border-slate-800/40">
            <CardTitle className="text-xl flex items-center gap-2">
              <Landmark className="h-5 w-5 text-primary" /> Enrolled Courses & Fee Details
            </CardTitle>
            <CardDescription className="text-slate-400">Breakdown of fees, payments, and outstanding balances for each of your enrolled courses.</CardDescription>
          </CardHeader>
          <CardContent className="p-6">
            <div className="space-y-4">
              {enrollments.map((course: any, index: number) => {
                ...
                return (
                  <div key={course.id || index} className="p-5 rounded-xl border border-slate-800/80 bg-slate-950/30 hover:border-slate-800 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <Badge className="bg-primary/10 text-primary border border-primary/20 mb-1">{course.course_code}</Badge>
                      <h3 className="font-bold text-slate-200 text-lg leading-tight">{course.batch_name || course.parent_course_name}</h3>
                    ...
                    <div className="grid grid-cols-3 gap-6 md:gap-12 text-center md:text-right shrink-0">
                      <div>
                        ...
                        <p className="text-sm font-semibold text-slate-350 mt-1">LKR {summary.totalFee.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground font-medium">Paid</p>
                        <p className="text-sm font-semibold text-green-500 mt-1">LKR {summary.totalPaid.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground font-medium">Due Balance</p>
                        <p className={`text-sm font-bold mt-1 ${summary.due > 0 ? "text-rose-500" : "text-green-500"}`}>
                          LKR {summary.due.toLocaleString(undefined, {minimumFractionDigits: 2})}
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <Card className="shadow-md border-yellow-500/10">
            <CardHeader className="bg-gradient-to-r from-yellow-500/10 to-transparent border-b border-slate-800/40">
              <CardTitle className="flex items-center gap-2 text-xl text-yellow-500">
                <History className="h-5 w-5" /> Pending Verification
              </CardTitle>
              <CardDescription className="text-slate-400">Slips submitted by you that are awaiting admin approval.</CardDescription>
            </CardHeader>
            <CardContent className="p-6">
              {pendingPayments.length > 0 ? (
                <div className="space-y-4">
                  {pendingPayments.map((req: any) => (
                    <div key={req.id} className="flex justify-between items-center p-4 rounded-xl bg-slate-950/40 border border-slate-800/80">
                      <div>
                        <p className="font-bold text-yellow-500 text-sm">Ref: {req.payment_reference || req.unique_number}</p>
                      ...
                      <div className="text-right">
                        ...
                        <Badge className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 mt-1 uppercase text-[9px] tracking-wider font-bold">Pending</Badge>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <Card className="shadow-md">
            <CardHeader className="border-b border-slate-800/40">
              ...
              <CardDescription>All approved payments credited to your account.</CardDescription>
            </CardHeader>
            <CardContent className="p-6">
              {paymentRecordsList.length > 0 ? (
                <div className="space-y-4">
                  {paymentRecordsList.map((rec: any) => (
                    <div key={rec.id} className="flex justify-between items-center p-4 rounded-xl bg-slate-950/20 border border-slate-800/80">
                      <div>
                        <p className="font-bold text-slate-200 text-sm">{rec.receipt_number}</p>
                      ...
                      <div className="text-right">
                        ...
                        <Badge className="bg-green-500/10 text-green-500 border border-green-500/20 mt-1 uppercase text-[9px] tracking-wider font-bold">Approved</Badge>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="lg:col-span-5">
          <Card className="shadow-xl border-primary/20 relative overflow-hidden h-fit">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-3xl pointer-events-none"></div>
            <CardHeader className="border-b border-slate-800/40">
              <CardTitle className="flex items-center gap-2 text-xl">
                <Upload className="h-5 w-5 text-primary" /> Upload Installment Slip
              </CardTitle>
              <CardDescription>Submit a new payment slip to settle your outstanding balance.</CardDescription>
            </CardHeader>
            <CardContent className="p-6">
              {balance && balance.studentBalance <= 0 ? (
                <div className="text-center py-8 space-y-3">
                  ...
                  <h3 className="font-semibold text-slate-200">No Outstanding Balance</h3>
                ...
                <form onSubmit={handleSubmitSlip} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="amount" className="text-sm font-medium text-slate-300">Amount Paid (LKR) *</Label>
                    <Input 
                      id="amount" 
                      type="number"
                      step="0.01"
                      ...
                      required 
                      className="bg-slate-950 border-slate-800 focus:border-primary text-slate-100 placeholder:text-slate-700" 
                      placeholder="e.g. 9000"
                    />
                  </div>
                  <div className="space-y-2 flex flex-col">
                    <Label htmlFor="bank" className="text-sm font-medium text-slate-300">Bank Name *</Label>
                    <BankSelector />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="branch" className="text-sm font-medium text-slate-300">Branch Name (Optional)</Label>
                      <Input 
                        id="branch" 
                        value={branch} 
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="paymentReference" className="text-sm font-medium text-slate-300">Reference Number (Optional)</Label>
                      <Input 
                        id="paymentReference" 
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-slate-300">Payment Slip *</Label>
                    <div className="border-2 border-dashed border-slate-800 hover:border-primary/50 transition-all rounded-xl p-4 text-center cursor-pointer relative bg-slate-950/40">
                      <input 
                        type="file" 
                      ...
                      <div className="flex flex-col items-center justify-center space-y-1">
                        <Upload className="w-8 h-8 text-slate-400 group-hover:text-primary mb-1" />
