...
import { getCourseContentModules, getCourseContentById, updateCourseContent, getBatches, uploadCourseContentFile } from "@/lib/actions/courses";
import type { CourseContentModule, CourseContent, Batch } from "@/lib/types";
...
export default function EditRecordingPage() {
  ...
  const { data: content, isLoading: isLoadingContent } = useQuery<CourseContent>({
    queryKey: ['courseContentSingle', contentId],
  ...
  useEffect(() => {
    if (content) {
        ...
        let resType = content.resource_type;
        ...
        if (desc.includes("<iframe") && desc.includes("youtube.com/embed")) {
            ...
            if (srcMatch && srcMatch[1]) {
             ...
             resType = "External Link"; // Fallback for unknown legacy types
        }

        setResourceType(resType);
        setDescription(desc);
        setWebLink(link);
        setFilePath(content.file_path || "");
    }
  }, [content]);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => updateCourseContent(contentId, data),
    onSuccess: () => {
      toast({ title: "Success", description: "Course content updated." });
      router.push("/admin/recordings");
    },
    onError: () => {
      toast({ variant: "destructive", title: "Error", description: "Failed to update course content." });
    }
  });
    ...
    <div className="p-4 md:p-8 space-y-6 pb-20 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <Card className="shadow-lg border-t-4 border-t-primary">
            <CardHeader>
              <CardTitle className="text-2xl font-headline">Edit Course Content</CardTitle>
              <CardDescription>Update the details of the learning material.</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <Button type="submit" className="w-full h-12 text-lg font-semibold mt-4" disabled={updateMutation.isPending}>
                  {updateMutation.isPending ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : "Update Content"}
                </Button>
              </form>
            </CardContent>
          </Card>
