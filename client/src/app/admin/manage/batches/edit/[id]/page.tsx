...
import { getParentCourseList, updateBatch, getBatches } from '@/lib/actions/courses';
...
export default function EditBatchPage() {
    ...
    useEffect(() => {
        if (batch) {
            form.reset({
                ...batch,
                parent_course_id: batch.parent_course_id ? String(batch.parent_course_id) : '',
                fee: batch.fee ? parseFloat(batch.fee) : 0,
                registration_fee: batch.registration_fee ? parseFloat(batch.registration_fee) : 0,
    ...
    const updateMutation = useMutation({
        mutationFn: (data: BatchFormValues) => updateBatch(batchId, data),
    ...
    if (isLoadingBatches || isLoadingParentCourses || isLoadingCriteria) {
        return <div className="p-8 flex justify-center items-center"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>;
    }
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 pb-20">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-headline font-semibold">Edit Batch: {batch?.name}</h1>
                    <p className="text-muted-foreground">Modify the details for this batch.</p>
                </div>
            </div>
