...
export default function WinPharmaGradingPage() {
        ...
        if (submission) {
            ...
            if (rawReason) {
                ...
                if (ids.length > 0) {
                    ...
                    // Any non-ID part would be custom text if we had a separator, 
                    // otherwise we just load it all as notes if no IDs found
                } else {
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['adminWinPharmaSubmissions'] });
            queryClient.invalidateQueries({ queryKey: ['adminWinPharmaSubmission', submissionId] });
            
            // Navigate back to the course-specific dashboard (e.g., /submissions/PCC26)
            const courseCode = (submission as any)?.course_code;
        ...
        <div className="min-h-screen bg-black flex flex-col">
            ...
            <header className="p-4 md:p-6 bg-zinc-950 border-b border-white/5 flex items-center justify-between sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" onClick={() => router.push('/admin/manage/winpharma-submissions')} className="h-10 w-10 md:h-12 md:w-12 rounded-xl bg-white/5 hover:bg-white/10 text-white">
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                    <div>
                        <h1 className="text-lg md:text-xl font-black text-white tracking-tight">Reviewing Submission</h1>
                        <p className="text-[10px] md:text-xs font-bold text-zinc-500 uppercase tracking-widest">{submission.index_number} • Level {submission.level_id}</p>
                    </div>
                </div>
                ...
                <div className="flex-1 bg-zinc-900/50 p-4 md:p-8 flex flex-col overflow-hidden relative">
                    <div className="flex-1 relative bg-zinc-950 rounded-[2.5rem] shadow-2xl border border-white/5 flex items-center justify-center overflow-auto p-4 md:p-8 m-auto w-full group">
                        ...
                        <div className="absolute top-8 right-8 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="bg-black/80 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 text-[10px] font-black uppercase text-zinc-400">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                ...
                                {filteredReasons.length === 0 && (
                                    <div className="py-20 text-center space-y-4 opacity-20">
                                        ...
                                        <p className="font-black text-lg uppercase tracking-widest">No Matches</p>
                                    </div>
                                )}
                            </div>
                        </ScrollArea>
