...
export default function EditBlogPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchBlog = async () => {
            try {
                const res = await fetch(`http://localhost/sos-full-stack/server/api/blogs/id/${params.id}`);
...
<p>Implementing structured processes for ${customTopic} leads to decreased medical errors and higher satisfaction levels. At Ceylon Pharma College, we prioritize these topics within our core curriculums to prepare students for real-world demands.</p>`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`http://localhost/sos-full-stack/server/api/blogs/${params.id}`, {
                method: "PUT",
    ...
    if (fetching) return <div className="flex h-screen items-center justify-center bg-gray-50"><Loader2 className="w-8 h-8 animate-spin text-emerald-600" /></div>;
            ...
            <div className="sticky top-0 z-30 bg-white border-b px-6 py-4 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500">Updating existing article</p>
                    </div>
                </div>
