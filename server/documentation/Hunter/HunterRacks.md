# Hunter Racks API Documentation

## Overview

The Hunter Racks API provides CRUD operations for managing racks within the system. This documentation outlines the available endpoints, request parameters, response structures, and error codes.

## Base URL

`https://api.pharmacollege.lk/hunter-racks`

## Endpoints

### 1. Get All Racks

- **Endpoint:** `GET /hunter-racks/`
- **Description:** Retrieves a list of all racks.
- **Response:**
  - **Status Code:** `200 OK`
  - **Body:**
    ```json
    [
      {
        "id": 1,
        "rack_name": "Rack 1",
        "active_status": "Active",
        "created_at": "2024-09-01T12:34:56Z",
        "created_by": "admin"
      },
      {
        "id": 2,
        "rack_name": "Rack 2",
        "active_status": "Inactive",
        "created_at": "2024-09-02T13:00:00Z",
        "created_by": "user1"
      }
    ]
    ```

### 2. Get Rack by ID

- **Endpoint:** `GET /hunter-racks/{id}/`
- **Description:** Retrieves a specific rack by its ID.
- **Path Parameters:**
  - `id` (integer): The ID of the rack to retrieve.
- **Response:**
  - **Status Code:** `200 OK`
  - **Body:**
    ```json
    {
      "id": 1,
      "rack_name": "Rack 1",
      "active_status": "Active",
      "created_at": "2024-09-01T12:34:56Z",
      "created_by": "admin"
    }
    ```
  - **Status Code:** `404 Not Found` (if the rack does not exist)
  - **Body:**
    ```json
    {
      "error": "Record not found"
    }
    ```

### 3. Create a New Rack

- **Endpoint:** `POST /hunter-racks/`
- **Description:** Creates a new rack.
- **Request Body:**
  - **Content-Type:** `application/json`
  - **Body:**
    ```json
    {
      "rack_name": "New Rack",
      "active_status": "Active",
      "created_by": "admin"
    }
    ```
- **Response:**
  - **Status Code:** `201 Created`
  - **Body:**
    ```json
    {
      "message": "Record created successfully"
    }
    ```

### 4. Update a Rack

- **Endpoint:** `PUT /hunter-racks/{id}/`
- **Description:** Updates an existing rack.
- **Path Parameters:**
  - `id` (integer): The ID of the rack to update.
- **Request Body:**
  - **Content-Type:** `application/json`
  - **Body:**
    ```json
    {
      "rack_name": "Updated Rack",
      "active_status": "Inactive",
      "created_by": "admin"
    }
    ```
- **Response:**
  - **Status Code:** `200 OK`
  - **Body:**
    ```json
    {
      "message": "Record updated successfully"
    }
    ```

### 5. Delete a Rack

- **Endpoint:** `DELETE /hunter-racks/{id}/`
- **Description:** Deletes a specific rack by its ID.
- **Path Parameters:**
  - `id` (integer): The ID of the rack to delete.
- **Response:**
  - **Status Code:** `200 OK`
  - **Body:**
    ```json
    {
      "message": "Record deleted successfully"
    }
    ```

## Error Codes

- **400 Bad Request:** The request was invalid or cannot be served.
- **404 Not Found:** The requested resource could not be found.
- **500 Internal Server Error:** An error occurred on the server.

## Example Requests

### cURL Example for Creating a Rack

```bash
curl -X POST https://api.pharmacollege.lk/hunter-racks \
-H "Content-Type: application/json" \
-d '{
  "rack_name": "New Rack",
  "active_status": "Active",
  "created_by": "admin"
}'
